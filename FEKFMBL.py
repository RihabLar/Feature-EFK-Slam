...
class FEKFMBL(GFLocalization, MapFeature):
    ...
    def h(self, xk):  # overloaded to stack measurements and feature observations
        ...
        * Feature Observations: :math:`z_f` correspond to observations of the features (CartesianFeature, PolarFeature, EsphericalFeature, etc...).
        ...
        # Compute expected measurements if orientation or encoder measurements are valid
        hm = self.hm(xk) if self.orientation_val or self.encoder_val else np.zeros((0, 1))

        # Compute expected feature observations if feature observations are valid
        if self.feature_val:
            # Filter out None values from self.H
            map_i = [h for h in self.H if h is not None]
            hf = self.hf(xk, map_i)
        else:
            hf = np.zeros((0, 1))

        # Stack the expected measurements and feature observations into a single vector
        h_mf = np.vstack([hm, hf])
        return h_mf
