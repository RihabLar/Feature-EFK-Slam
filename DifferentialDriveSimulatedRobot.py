...
class DifferentialDriveSimulatedRobot(SimulatedRobot):
    ...
    def __init__(self, xs0, map=[],*args):
        ...
        * **Rfp** : Object attribute containing the covariance of the simulated Polar2D feature noise (:math:`R_{fp}=diag(\\sigma_{\\rho}^2,\\sigma_{\\phi}^2)`)
        ...
        self.Cartesian_feature_reading_frequency = 50  # Frequency of Cartesian2D feature readings
        ...
        self.yaw_reading_frequency = 10000  # Frequency of Yaw readings
    ...
    def ReadCartesianFeatureObservation(self):
        ...
        for i in range(0,self.nf):
            noise = np.zeros((len(self.Rfc), 1))  # Set noise to zero
            # Compute the observation with added noise and append to zsk
            observation = self.M[i].boxplus(xsk.ominus()) + noise
            zsk.append(observation)

            # Append the covariance matrix to Rsk
            Rsk.append(self.Rfc)
